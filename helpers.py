...
class Args: # pylint: disable-msg=R0902
    ...
    def __init__(self):
        ...
        if self.silent and self.user is None:
            self._user = getpass.getuser().title()
    ...
    def overwrite_csv(self) -> bool:
        ...
        Can only be set once, to avoid overwriting `True`
        when to_csv is provided through `config.ini`.
        Will block attempt to re-set if already pre-set.
        """
...
def parse_args() -> dict:
    ...
    mdb_server_group.add_argument(
        '-u', '--user',
        help = "GnuCash MariaDB username"
    )
    mdb_server_group.add_argument(
        '--pwd',
        help = "GnuCash MariaDB password. " +
    ...
    yf_group.add_argument(
        ...
        help = "Data period to download (either use period parameter or use start and end)." +
            "'auto' will determine start date based on last available price date"
    )
